...
public class FileSystemTool implements MCPTool {
    ...
    public String getDescription() {
        switch (toolName) {
            ...
            case "du": return "计算目录或文件的大小（类似 du 命令）";
            case "hash": return "计算文件的哈希值（MD5、SHA-1、SHA-256）";
        ...
        try {
            ...
            switch (toolName) {
                ...
                case "copy": case "rename": {
                    ...
                    JSONObject f = new JSONObject(); f.put("type", "boolean"); f.put("description", "是否强制覆盖已存在文件"); f.put("default", false); props.put("force", f);
                ...
                case "edit": {
                    JSONObject p = new JSONObject(); p.put("type", "string"); p.put("description", "文件路径"); props.put("path", p);
                    JSONObject f = new JSONObject(); f.put("type", "string"); f.put("description", "要查找的文本（支持正则表达式）"); props.put("find", f);
                    JSONObject r = new JSONObject(); r.put("type", "string"); r.put("description", "替换为的文本"); props.put("replace", r);
                    JSONObject re = new JSONObject(); re.put("type", "boolean"); re.put("description", "是否使用正则表达式"); re.put("default", false); props.put("regex", re);
                    schema.put("required", new JSONArray().put("path").put("find").put("replace")); break;
                }
                case "du": {
                    JSONObject p = new JSONObject(); p.put("type", "string"); p.put("description", "目录或文件路径"); props.put("path", p);
                    schema.put("required", new JSONArray().put("path")); break;
                }
                case "hash": {
                    JSONObject p = new JSONObject(); p.put("type", "string"); p.put("description", "文件路径"); props.put("path", p);
                    JSONObject a = new JSONObject(); a.put("type", "string"); a.put("description", "哈希算法: MD5, SHA-1, SHA-256"); a.put("default", "MD5"); props.put("algorithm", a);
                    schema.put("required", new JSONArray().put("path")); break;
                }
            }
    ...
    public JSONObject execute(Context context, JSONObject args) throws Exception {
        switch (toolName) {
            ...
            case "du": return execDu(args);
            case "hash": return execHash(args);
    ...
    private void buildTree(File dir, String prefix, StringBuilder sb, int maxDepth, int depth) {
        ...
        for (int i = 0; i < files.length; i++) {
            if (count >= 1000) { // 限制最大文件数
                sb.append(prefix).append("... (更多文件未显示)\n");
                break;
            }
    ...
    private void findFiles(File dir, String name, JSONArray results) {
        findFiles(dir, name, results, 0, 50);
    }

    private void findFiles(File dir, String name, JSONArray results, int depth, int maxDepth) {
        ...
        for (File f : files) {
            try {
                ...
                Log.w("FileSystemTool", "findFiles item error: " + e.getMessage());
            }
            if (f.isDirectory()) findFiles(f, name, results, depth + 1, maxDepth);
        }
    }
    ...
    private void grepFiles(File file, String query, JSONArray results) {
        grepFiles(file, query, results, 0, 50);
    }

    private void grepFiles(File file, String query, JSONArray results, int depth, int maxDepth) {
        if (depth >= maxDepth) return;
        if (file.isDirectory()) {
            File[] files = file.listFiles();
            if (files == null) return;
            for (File f : files) grepFiles(f, query, results, depth + 1, maxDepth);
            return;
        }
        ...
        try (java.io.BufferedReader br = new java.io.BufferedReader(new java.io.FileReader(file))) {
            ...
            Log.w("FileSystemTool", "grepFiles error: " + e.getMessage());
        }
    }
    ...
    private JSONObject execCopy(JSONObject args) throws Exception {
        ...
        if (dstFile.exists() && !force) throw new Exception("目标已存在: " + dst + " (使用 force=true 覆盖)");
    ...
    private void copyRecursive(File src, File dst) throws Exception {
        copyRecursive(src, dst, 0, 100);
    }

    private void copyRecursive(File src, File dst, int depth, int maxDepth) throws Exception {
        if (depth >= maxDepth) return;
        if (src.isDirectory()) {
            dst.mkdirs();
            File[] children = src.listFiles();
            if (children != null) for (File c : children) copyRecursive(c, new File(dst, c.getName()), depth + 1, maxDepth);
        } else {
    ...
    private void deleteRecursive(File f) {
        deleteRecursive(f, 0, 100);
    }

    private void deleteRecursive(File f, int depth, int maxDepth) {
        if (depth >= maxDepth) return;
        if (f.isDirectory()) {
            File[] children = f.listFiles();
            if (children != null) for (File c : children) deleteRecursive(c, depth + 1, maxDepth);
        }
        f.delete();
    }
    ...
    private JSONObject execEdit(JSONObject args) throws Exception {
            ...
            String replace = args.getString("replace");
            boolean regex = args.optBoolean("regex", false);
            File f = new File(path);
            if (!f.exists()) throw new Exception("文件不存在: " + path);
            if (find.isEmpty()) throw new Exception("查找文本不能为空");
            ...
            String newContent;
            if (regex) {
                java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(find);
    ...
    private JSONObject execDu(JSONObject args) throws Exception {
        String path = session.resolve(args.getString("path"));
        File f = new File(path);
        if (!f.exists()) throw new Exception("路径不存在: " + path);

        long[] sizes = calculateDirectorySize(f, 0, 50);
        JSONObject r = new JSONObject();
        r.put("path", f.getAbsolutePath());
        r.put("total_size", sizes[0]);
        r.put("file_count", sizes[1]);
        r.put("dir_count", sizes[2]);
        r.put("size_human", formatSize(sizes[0]));
        return r;
    }

    private long[] calculateDirectorySize(File dir, int depth, int maxDepth) {
        long[] result = new long[]{0, 0, 0}; // totalSize, fileCount, dirCount
        ...
        for (File f : files) {
            if (f.isDirectory()) {
                result[2]++;
                long[] sub = calculateDirectorySize(f, depth + 1, maxDepth);
                result[0] += sub[0];
                result[1] += sub[1];
                result[2] += sub[2];
            } else {
    ...
    private JSONObject execHash(JSONObject args) throws Exception {
        String path = session.resolve(args.getString("path"));
        String algorithm = args.optString("algorithm", "MD5").toUpperCase();
        File f = new File(path);
        if (!f.exists()) throw new Exception("文件不存在: " + path);
        if (!f.isFile()) throw new Exception("不是文件: " + path);
